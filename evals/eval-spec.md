...
- 2026-07-22 — v1.4: the three residual priority failures classified after C5 (commit `bada1a0`). All four §3 ship bars now clear in every run (measured 3-run aggregate: mean 21.3/26, band 21–22); the remaining per-ticket failures are recorded here by kind. No expected answers, KB articles, or grader code changed.
  - **T-004 — KB-000 internal contradiction (corpus defect, deliberately not fixed).** The medium row's example list contains "printer offline" while the row's definition reads "single user degraded or blocked." A whole-office printer matches the example but not the definition, so the model matches the row and then applies the "+1 multiple users" modifier; C5's no-stacking clause cannot bite because the row's definition claims single-user impact only. This is a corpus defect discovered by the eval process. It is explicitly not fixed here: disambiguating KB-000 while a golden ticket depends on the ambiguity is teaching to the test. Deferred to independent review alongside the negation-aware `must_not_include` decision (v1.2).
  - **T-008 — genuinely ambiguous boundary (borderline judgment, not an agent defect).** Whether losing a second monitor is "work degraded" (medium) or "accessory lost, primary work continues" (low) is defensible either way. The golden answer says low; the model consistently argues medium, in every run, before and after C5's impeded-vs-inconvenienced rule addressed exactly this case.
  - **T-012 — lead-time clause did not bite.** The model treats "starting Monday" as a deadline modifier regardless of KB-007's stated 3-business-day lead time, leaving the ticket at high (want medium) in every run. The C5 rule stating that a future date within the KB's lead time is not a same-day deadline did not change this behavior.
  - **Corroboration of v1.3.** C5 touched only priority text, and T-010's draft phrasing still reshuffled (`manager approval` dropped in 2 of 3 runs) — unprompted corroboration of v1.3's structural conclusion that any prompt change reshuffles draft wording, and the reason no further prompt iteration was attempted on these residuals.
